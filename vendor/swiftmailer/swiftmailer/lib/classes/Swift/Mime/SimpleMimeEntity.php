...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_MimeEntity
{
    ...
    {
        $this->_cacheKey = md5(uniqid(mt_rand(0, 32000), true));  // md5(uniqid(getmypid().mt_rand(), true)); disabled by host due to security reasons.
        $this->_cache = $cache;
    ...
    {
        $idLeft = md5(mt_rand(0, 32000) . '.' . time() . '.' . uniqid(mt_rand(), true));    // md5(getmypid().'.'.time().'.'.uniqid(mt_rand(), true)); Disabled by host due to security reasons
